...
const ProductCard: React.FC<{ product: ProductType }> = ({ product }) => {
  ...
  const component = useMemo(
    () => (
      <div className="overflow-hidden bg-white rounded-lg font-roboto w-72">
        <div className="m-2">
          <img
            src={product?.p_image}
            alt={product?.p_name}
        ...
        <div className="h-24 -mt-2">
          <p className="px-2 text-sm text-gray-400 font-pitagon-400">
            {product?.p_description.short || product.p_description.storage_tips}
          </p>
        </div>
        <div className="flex items-center justify-center px-2 mx-auto mt-2 text-white bg-red-600 w-max">
          <h4 className="px-2 font-semibold rounded-sm">Price:</h4>

          <h4 className="text-lg font-semibold ">₹ {product?.p_price}</h4>
        </div>
